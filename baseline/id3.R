...
data$created <- format(as.Date(as.character(data$created)), "%Y")
...
id3 <- function(important_data){
	...
	prediction <- tryCatch({
		j <- ID3(timespent ~ ., important_data_train)
...
calculate <- function(i){
	...
	bug.predict.classonly <- id3(involved_data)
	
	print('algorythm done')
	write.csv2(as.matrix(bug.predict.classonly),paste(error_ranges_graph, "bug.predict",i,".csv",sep=""))
	bug.predict.classonly.aerr <- diff(bug.predict.classonly, rt_data_actual[(1 + break_point):nrow(rt_data),2])
	if (is.null(bug.predict.classonly.aerr)) {
		bug.predict.classonly.aerr = NA
	}
...
print(all_data_path)
	...
	PRED <- data.frame(nrow(rM[rM$rel <= 0.1, ]), nrow(rM[rM$rel <= 0.25, ]), 
		nrow(rM[rM$rel <= 0.33, ]), nrow(rM[rM$abs <= 1, ]), nrow(rM[rM$abs <= 3, ]))
	colnames(PRED) <- c("0.1", "0.25", "0.33", "1h", "3h")
	PRED <- PRED/nrow(rM)
}
}
