...
decision_tree <- function(important_data){
	...
	prediction <- tryCatch({
		fit <- train(timespent~., data=important_data_train, method = "treebag")
		#fit <- ctree(timespent ~ ., data=important_data_train)
		predict(fit, important_data_new)
	}, error = function(e){
		print(e)
		NA
	})
...
calculate <- function(i){
	...
	bug.predict.classonly <- decision_tree(involved_data)
