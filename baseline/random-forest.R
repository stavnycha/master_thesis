...
without_sumbols <- function(string) {
	return(str_replace_all(string, "[^[:alnum:]]", ""))
}

reporters_to_columns <- function(important_data){
	reporters <- unique(important_data$reporter)
	for (reporter in reporters) {
		important_data[, without_sumbols(reporter)] <- important_data$reporter == reporter
	}
...
random_forest <- function(important_data){
	...
	prediction <- tryCatch({
		...
		#rf <- train(timespent~., data=important_data_train, method = "parRF", 
		#	tuneGrid = data.frame(mtry=mtry))
		#rf <- train(timespent~., data=important_data_train, method = "rf", 
