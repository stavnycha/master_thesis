...
only_bayes_classifier <- function(important_data){
	...
	nb <- naiveBayes(timespent ~ ., data=important_data_train)
	predict <- predict(nb, important_data_new)
	return(predict)
}
...
calculate <- function(i){
	...
	bug.predict.classonly <- only_bayes_classifier(involved_data)
...
print(all_data_path)
	...
	shortened_table <- rM[rM$model == "AE < 1",] # whatever model, just 1 line per result is needed
